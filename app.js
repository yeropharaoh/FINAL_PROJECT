    ...
    if (state === null || state !== storedState) {
        ...
        request.post(authOptions, function(error, response, body) {
            if (!error && response.statusCode === 200) {
                ...
                // uses the access token to access the Spotify Web API
                request.get(options, function(error, response, body) {
                    console.log(body);
                });

                // token to the browser can also be passed to make requests from there
...
// Emotion API x Spotify API Page
...
app.post('/search', (request, response) => {
    ...
    rapid.call('MicrosoftEmotionAPI', 'getEmotionRecognition', {
        ...
        // The loop below finds the emotion detected.
